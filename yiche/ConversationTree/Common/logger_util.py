...
from ConversationTree.Common.parameterize_util import get_object_path, read_config
import logging
import datetime
...
class LoggerUtil:
    def create_log(self, logger_name='log'):
        ...
        if not self.logger.handlers:
            ...
            #     int(time.time())) + ".log"
            self.file_log_path = get_object_path() + "/Logs/" + str(
                datetime.date.today()) + ".log"
            # 2.创建文件日志的控制器
            self.file_hander = logging.FileHandler(
                self.file_log_path, encoding='utf-8')
            # 3.创建文件日志的日志级别
            ...
            if console_log_level == "debug":
                ...
                self.console_hander.setLevel(logging.DEBUG)
            # 3.创建控制台日志格式
            self.console_hander.setFormatter(
                logging.Formatter(
                    read_config(
                        "log", "log_format")))
            # 将控制台日志的控制器加入到日志对象
